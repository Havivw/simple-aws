...
def AMI_details():# dict 5 min
    if current_user.is_authenticated:
        while True:
            try:
                with open("AMI_details","rb") as f:
                    all_images = pickle.load(f)
                    break
            except Exception as e:
                print("AMI_details Exception %s, trying again in 1 second"%e)
                time.sleep(1)
...
def inst_page():
    if current_user.is_authenticated:
        if request.method == 'POST':
            ...
            inst(user=username, verbose=False, spot=Spot, key=KeyPair, region=Site, ami=InstanceID, instancetype=EC2Types, SecurityGroup=SecurityGroups, IamInstanceProfile=InstanceProfiles, name=MachineName, prod=Production)
            all_instances = []
            start = timeit.default_timer()
            instances_info = get_instances()
            for region, instances in instances_info.items():
...
def start_EC2():#Take_Time
    if current_user.is_authenticated:
        start = timeit.default_timer()
        buckets = get_s3_bucket_names(request.args.get('site')) # dict 5 min
        print('buckets Time: ', timeit.default_timer() - start)

        start = timeit.default_timer()
        instance_profiles = get_instance_profiles_names(request.args.get('site')) # dict 5 min
        print('instance_profiles Time: ', timeit.default_timer() - start)

        start = timeit.default_timer()
        security_groups = get_all_SG(request.args.get('site')) # dict 5 min
        print('security_groups Time: ', timeit.default_timer() - start)

        start = timeit.default_timer()
        keypairs = get_keypairs_details(request.args.get('site')) # dict 5 min
        ...
        while True:
            try:
                with open("%s_%s" % (os, "get_all_EC2_types"), 'rb') as f:
                    EC2_types = pickle.load(f)
                    break
            except Exception as e:
                print("start_EC2 Exception %s, trying again in 1 second"%e)
                time.sleep(1)


        #EC2_types = get_all_EC2_types(os, request.args.get('site'), all=False)  # Take_Time if all = True (false in progress) # dict 5 min
